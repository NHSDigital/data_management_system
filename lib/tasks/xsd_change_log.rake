    ...
    task cosd_v8_v9_diffs_rawtext: :environment do
      ...
      CSV.open(filename, 'r+') do |csv|
        ...
        rows.each do |row|
          ...
          row_etype = row_etype.nil? ? '' : row_etype[0]
          etype_row << [row[0], row[1], row[2], row[3], row[4], row[5], row_etype, row[6]]
        end

        CSV.new('v8_v9_diffs_rawtext.csv')
        CSV.open('v8_v9_diffs_rawtext.csv', 'wb') do |out_csv|
          out_csv << ['V8 only rawtext_name', 'V8 only code', 'V8 only fieldname',
