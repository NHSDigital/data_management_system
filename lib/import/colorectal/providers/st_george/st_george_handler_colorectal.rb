        ...
        class StGeorgeHandlerColorectal < Import::Germline::ProviderHandler
          ...
          def process_r211(record)
            ...
            if date < DateTime.parse('18/07/2022')
              r211_panel_genes = %w[APC BMPR1A EPCAM MLH1 MSH2 MSH6 MUTYH NTHL1 PMS2 POLD1 POLE PTEN
                                    SMAD4 STK11]
            elsif date >= DateTime.parse('18/07/2022')
              r211_panel_genes = %w[APC BMPR1A EPCAM MLH1 MSH2 MSH6 MUTYH NTHL1 PMS2 POLD1 POLE PTEN
                                    SMAD4 STK11 GREM1 RNF43]
            end
            r211_panel_genes
          end
          ...
          def interrogate_variant_dna_targeted(record, genotype, genes, column, gene)
            ...
            elsif record.raw_fields['variant dna'].match(%r{^Normal|^no del/dup$}ix)
              genotype.add_status(1)

            elsif record.raw_fields['variant dna'].match(%r{SNP present$|^no del/dup$}ix)
              genotype.add_status(4)
