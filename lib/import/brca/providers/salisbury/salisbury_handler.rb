...
module Salisbury
  ...
  class SalisburyHandler < Import::Brca::Core::ProviderHandler
    ...
    COLORECTAL_GENES_REGEX = /(?<colorectal>APC|BMPR1A|EPCAM|MLH1|MSH2|MSH6|MUTYH|PMS2|POLD1|POLE|PTEN|SMAD4|STK11)/i.freeze # Added by
    ...
    def process_fields(record)
      ...
      add_colorectal_from_raw_test(genotype, record) # Added by Francesco
    ...
    def add_colorectal_from_raw_test(genotype, record)
      ...
      if colo_string.scan(COLORECTAL_GENES_REGEX).size > 1
        ...
        genotype.add_gene_colorectal($LAST_MATCH_INFO[:colorectal])
        # when COLORECTAL_GENES_REGEX and test_string.scan(GENE_REGEX).size > 1
        @logger.debug "SUCCESSFUL cdna change parse for: #{$LAST_MATCH_INFO[:colorectal]}"
      else
        @logger.debug "Could not parse #{colo_string}"
      end
    end
